...
public:
  ...
  virtual void Execute(vtkObject *caller, unsigned long, void*)
    ...
    float value = static_cast<float>(static_cast<vtkSliderRepresentation *>(slidrWidgt->GetRepresentation())->GetValue());
...
int main(int argc, char *argv[])
{
  ...
  slidrRep->GetSliderProperty()->SetColor(1,1,0);
  slidrRep->GetTubeProperty()->SetColor(0,1,1);
  slidrRep->GetCapProperty()->SetColor(0,1,0);
  slidrRep->SetSliderLength(0.02);
  slidrRep->SetSliderWidth(0.03);
  slidrRep->SetEndCapLength(0.01);
  slidrRep->SetEndCapWidth(0.03);
  slidrRep->SetTubeWidth(0.012);
  ...
  callback->normalGenerator = normalGenerator;
  callback->stripr = Stripper;
  callback->mappr = mappr1;
  callback->rendwind = rendwind;
